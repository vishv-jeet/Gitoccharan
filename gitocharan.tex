\documentclass[12pt]{book}
\usepackage[margin=1in]{geometry}
\usepackage{fontspec}
%\usepackage{polyglossia}
\usepackage{graphicx}
\usepackage[english]{babel} 
%\usepackage{devanagari}
%\newfontfamily\sanskrit[Script=Devanagari]{Lohit Devanagari}

%Setting Marathi as main language and Shobhika as main font using package Polyglossia.
%\usepackage{fontspec}
%\setmainfont[Script=Devanagari, Mapping=devanagarinumerals]{Lohit Devanagari}
%\newfontfamily\marathifont[Script=Devanagari, Mapping=devanagarinumerals]{Lohit Devanagari}
%\usepackage{polyglossia}
%\setmainlanguage[numerals=Devanagari]{Sanskrit}

\newfontfamily\sanskrit[Script=Devanagari]{Lohit Devanagari}
%\newfontfamily\roman[Script=Roman]{Times New Roman}
%\newcommand{\R}{\textup{\textrm{R}}}

%\usepackage[roman]{babel}

\usepackage{devanagaridigits}
\usepackage{tocloft}
\renewcommand{\cftchapdotsep}{\cftdotsep}
\usepackage{hyperref}
%\usepackage{devanagari}
\usepackage{fancyhdr}
\pagestyle{fancy}

%\newcommand{\devanagarinumeral}[1]{%
%	\devanagaridigits{\number\csname c@#1\endcsname}}

% renew all representation of counters
%\renewcommand{\thesection}{\devanagarinumeral{section}}
%\renewcommand{\thepage}{\devanagarinumeral{page}}
%\renewcommand{\theenumi}{\devanagarinumeral{enumi}}
%\renewcommand{\thechapter}{\devanagarinumeral{chapter}}

%\newcommand{\arabictodevnag}[1]%
%{%
%	\StrSubstitute{\num}{0}{\sanskrit ०}[\num]
%	\StrSubstitute{\num}{1}{\sanskrit १}[\num]
%	\StrSubstitute{\num}{2}{\sanskrit २}[\num]
%	\StrSubstitute{\num}{3}{\sanskrit ३}[\num]
%	\StrSubstitute{\num}{4}{\sanskrit ४}[\num]
%	\StrSubstitute{\num}{5}{\sanskrit ५}[\num]
%	\StrSubstitute{\num}{6}{\sanskrit ६}[\num]
%	\StrSubstitute{\num}{7}{\sanskrit ७}[\num]
%	\StrSubstitute{\num}{8}{\sanskrit ८}[\num]
%	\StrSubstitute{\num}{9}{\sanskrit ९}[\num]
%}

%\fancyhead{} % clear all header fields
\renewcommand{\headrulewidth}{0pt} % no line in header area
\fancyhead[LE,RO]{\sanskrit गीतोच्चारण}
%\fancyfoot{} % clear all footer fields
%\fancyfoot[LE,RO]{\thepage}           % page number in "outer" position of footer line
\fancyfoot[RE,LO]{\tiny \bf Copyrighted Material, All Rights Reserved} % other info in "inner" position of footer line

%\cfoot{\fancyplain{}{\Large \arabictodevnag{\thepage}}}

\usepackage{eso-pic}
\AddToShipoutPictureBG*{%
	\AtTextLowerLeft{\makebox[\textwidth][r]{%
			\includegraphics[scale=0.4,keepaspectratio]{978-1-63795-456-0.eps}}}}



\begin{document}
\def\chaptername{{\sanskrit अध्याय}}
\def\contentsname{{\sanskrit विषय सूची}}
\include{introduction.tex}
\tableofcontents
\include{chapter1.tex}
\include{chapter2.tex}
\include{chapter3.tex}
\include{chapter4.tex}
\include{chapter5.tex}
\include{chapter6.tex}
\include{chapter7.tex}
\include{chapter8.tex}
\include{chapter9.tex}
\include{chapter10.tex}
\include{chapter11.tex}
\include{chapter12.tex}
\include{chapter13.tex}
\include{chapter14.tex}
\include{chapter15.tex}
\include{chapter16.tex}
\include{chapter17.tex}
\include{chapter18.tex}

\begin{center}
ॐ

शान्ताकारं भुजग शयनं पद्मनाभं सुरेशम्  ।  

विश्वाधारं गगन सदृशं मेघवर्णं शुभाङ्गम्  ।  

लक्ष्मीकान्तं कमल नयनं योगिभि-अर्ध्यान-गम्यम्  ।  

वन्दे विष्णुं भव भयहरं सर्व-लोकैक-नाथम्  ॥ 

\end{center}
\newpage

\chapter*{}
\begin{center}
\includegraphics[scale=0.2,keepaspectratio]{eps3}
\end{center}

\AddToShipoutPictureBG*{%
	\AtTextLowerLeft{\makebox[\textwidth][r]{%
			\includegraphics[scale=0.4,keepaspectratio]{978-1-63795-456-0.eps}}}}

\end{document}


